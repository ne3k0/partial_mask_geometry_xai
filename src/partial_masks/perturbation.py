...
# Binary boundaries only: smoothed boundaries (Gaussian/Gabor) were shown to have no significant effect on attribution and so were removed. There is still room to explore their integration and to what extent they taper OOD behaviour.
...
def make_zero_fill(Zxx):
    def zero_fill(Zxx, stft_mask):
        return Zxx * stft_mask
    return zero_fill


def make_mean_fill(Zxx):
    fill = np.mean(np.abs(Zxx), axis=1, keepdims=True) * np.exp(1j * np.angle(Zxx))
...
def make_gaussian_noise_fill(Zxx):
    ...
    Zxx_noise = np.abs(rng.normal(0, mean_mag, Zxx.shape)) * np.exp(1j * np.angle(Zxx))
    def gaussian_noise_fill(Zxx, stft_mask):
        return stft_mask * Zxx + (1 - stft_mask) * Zxx_noise
    return gaussian_noise_fill


# FILL_FACTORIES: {name: Zxx -> fill_fn}
# Call factory(Zxx) once per clip to get fill_fn, then call fill_fn(Zxx, mask) per mask.
FILL_FACTORIES = {
    "zero": make_zero_fill,
    "mean": make_mean_fill,
    "gaussian_noise": make_gaussian_noise_fill,
}
