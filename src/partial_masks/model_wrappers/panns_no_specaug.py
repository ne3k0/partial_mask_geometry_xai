"""PANNs CNN14 without SpecAugment — custom-trained checkpoint."""
import platform
import os
import sys
...
CHECKPOINT = os.path.join(
    os.path.expanduser("~"), "checkpoints", "panns", "cnn14_no_specaug", "600000_iterations.pth"
)


class PannsModel:
    model_id = "PANNs/CNN14_no_specaug"

    def __init__(self):
        self.model = Cnn14_no_specaug(sample_rate=32000, window_size=1024, hop_size=320,
        ...
        self.class_to_idx = {v: k for k, v in self.labels.items()}  # label string → class index in the 527-dim output
    ...
    def infer_sigmoid_from_mel(self, log_mel_batch):
        """Single forward pass returning (predictions, (L, 527) sigmoid probs). No layer hooks."""
        output_dict = self._forward_from_mel(log_mel_batch)
        probs = output_dict['clipwise_output'].detach().cpu().numpy()
        return self._to_predictions(probs), probs
    ...
    def infer_all_layers_from_mel(self, log_mel_batch):
        ...
        layer_activations['sigmoid']      = output_dict['clipwise_output'].detach().cpu().numpy()  # (L, 527)
        preds = self._to_predictions(layer_activations['sigmoid'])
