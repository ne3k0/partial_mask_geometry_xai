...
class Model:
    def __init__(self, model_id) -> None:
        ...
        self.class_to_idx = {v: k for k, v in self.pipe.model.config.id2label.items()}  # label string → class index in the 527-dim output
    ...
    def infer_sigmoid_from_mel(self, log_mel_batch):
        """Single forward pass returning (predictions, (L, 527) sigmoid probs). No hidden states."""
        x = self._prepare_input(log_mel_batch)
        with tt.no_grad():
            output = self.pipe.model(input_values=x)
        probs = tt.sigmoid(output.logits).cpu().numpy()
    ...
    def infer_all_layers_from_mel(self, log_mel_batch):
        ...
        layer_embs['sigmoid'] = probs  # (L, 527)
